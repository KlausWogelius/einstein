...
\path (10,0) coordinate(C);
\path (10,4) coordinate(A);
\draw (B) -- node[midway, below      ]{$a$} (C)
          -- node[midway, right      ]{$b$} (A)
          -- node[midway, above left ]{$c$} (B);
\draw (9,0) -- (9,1) -- (10,1);
\end{tikzpicture}[scale=1]
...
\path (O)+(0:4cm) coordinate (A1); %definer objekt A1  
...
\draw [dashed, ->] (2,1) arc (0:80:1cm)         ;% marker, at l1=l2
